...
from utils import BartTripletHead, shift_tokens_left, extract_triplets_typed, extract_triplets, extract_triplets_selfmade
...
relations_selfmade = ['acquaintance', 'aunt/uncle', 'betrayed',
                      'cousin', 'defended', 'detests', 
                      'fought', 'friend', 'hates', 'helped', 'influencedBy', 'killed', 
                      'likes', 'loves', 'mentor', 'negotiated','nickname', 
                      'opposed', 'parent', 'saved', 'sibling', 
                      'sponsored', 'spouse', 'successor', 'supervisor', 'trusts']


relations_selfmade1 = ['acquaintance', 'admires', 'aunt/uncle', 'betrayed', 'workedWith',
...
class BasePLModule(pl.LightningModule):
    ...
    def generate_triples(self,
        ...
        elif self.hparams.dataset_name.split('/')[-1] == 'self_relations.py':
            return [extract_triplets_selfmade(rel) for rel in decoded_preds], [extract_triplets_selfmade(rel) for rel in decoded_labels]        
    ...
    def validation_epoch_end(self, output: dict) -> Any:
        if self.hparams.relations_file:
            ...
            if self.hparams.dataset_name.split('/')[-1] == 'conll04_typed.py':
                ...
                scores, precision, recall, f1 = re_score([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']], relations_selfmade)
            else:
    ...
    def test_epoch_end(self, output: dict) -> Any:
        if not self.hparams.finetune and self.hparams.relations_file:
            ...
            if self.hparams.dataset_name.split('/')[-1] == 'conll04_typed.py':
                ...
                scores, precision, recall, f1 = re_score([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']], relations_selfmade, "directionless")
            else:
                scores, precision, recall, f1 = re_score([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']], ['killed by', 'residence', 'location', 'headquarters location', 'employer'])
            self.log('test_prec_micro', precision)
            self.log('test_recall_micro', recall)
